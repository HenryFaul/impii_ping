      ...
      <div class="mt-4 w-full mt-3 mb-1">
        ...
        <div class="max-w-3xl w-full bg-white rounded-md shadow overflow-hidden mt-4 mr-2 mb-4 p-4">
              ...
              <td class="p-2">
                <div v-if="detail.agent_accepted">
                  ...
                  <Link class="group flex items-center py-3" :href="`/agent/${detail.agent_id}/profile`">
                    <div>
                      <span
                        class="text-xs inline-block py-1 px-2.5 leading-none text-center whitespace-nowrap align-baseline font-bold bg-impii underline text-white rounded-full">View agent</span>
                    </div>
                  </Link>
                </div>
                <span v-else
                      class="text-xs inline-block py-1 px-2.5 leading-none text-center whitespace-nowrap align-baseline font-bold bg-impii text-white rounded-full">pending</span>
              </td>
            </tr>
            <tr class="row">
          ...
          <div  v-if="detail.detail_closed">
            <div>Detail is closed...</div>
          </div>
          <div v-else>
            <div v-if="detail.detail_ended">
              <div class="bg-yellow-100 mt-2 mb-2 rounded-lg py-5 px-6 mb-2 text-base text-yellow-700 mb-3" role="alert">
                You need to pay the final amount due. Please add a tip if you feel your agent was awesome. Confirm to
                continue.
              </div>

              <div v-if="!form_data">
                <text-input v-model="form.tip_charge" class="pb-8 pr-6 w-2/3" label="Tip"/>
                <loading-button class="btn-impii" @click="update">Confirm details</loading-button>
              </div>

              <div v-else>
                <form id="payfast-pay-form" ref="payfast_pay_form" onsubmit="{alert('Redirecting to Payfast..')}"
                  ...
                  <loading-button :loading="submit_loading" class="btn-indigo" type="submit">Pay Final (Payfast)
                  </loading-button>
                </form>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    update() {
      this.form.post('/pay/update/tip', {
        onSuccess: () => this.updateInertia()
      })
    },
